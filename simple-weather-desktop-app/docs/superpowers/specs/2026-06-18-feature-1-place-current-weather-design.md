...
- **No matches** — friendly inline "Couldn't find that place — check the spelling and try again." (driven by an empty candidate array, not an error).
...
- **(d) proof:** A **contract round-trip on the real serializer output** crosses this boundary: a Rust test asserts that `serde_json::to_value(LocationCandidate { region: None, .. })` equals a committed golden fixture (`contract-fixtures/location_candidates.json`), and a frontend test parses that **same** fixture into the TS `LocationCandidate[]`, asserting the exact key set and `region === null`. The golden file is the real Rust writer's output (pinned by the Rust equality assertion), so the frontend consumes the genuine serialized shape — not a hand-authored literal. The geocode flow tests may still mock `invoke` (they test orchestration, not the wire shape); the wire contract is pinned solely by this round-trip. Owning symbols: `geocode` command fn + the `LocationCandidate` serde struct + the TS `LocationCandidate` type + `contract-fixtures/location_candidates.json`.
...
- **(d) proof:** A **contract round-trip on the real serializer output** crosses this boundary, which is the only place the `#[serde(rename_all = "camelCase")]` contract is provable: a Rust test asserts `serde_json::to_value(CurrentConditions { .. })` equals a committed golden fixture (`contract-fixtures/current_conditions.json`), and a frontend test parses that **same** fixture into the TS `CurrentConditions`, asserting the exact camelCase key set (`temperatureC`, `weatherCode`, `conditionLabel`). The golden file is the genuine Rust serializer output (pinned by the Rust assertion), so a future change that drops `rename_all` turns the Rust test red and the frontend key-set assertion red — neither side can drift silently. Icon-mapping coverage (no WMO code renders blank) is proven separately in the `weatherIcons` test. The fetch flow tests may still mock `invoke`. Owning symbols: `fetch_current` command fn + `CurrentConditions` serde struct + TS type + `contract-fixtures/current_conditions.json` + the code→label table (Rust) and code→icon table (frontend).
